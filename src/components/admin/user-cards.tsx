...
const UserCards = ({ users }: UsersCardsProps) => {
  return (
    <Card className="user-cards">
      ...
      <div>
        <p className="user-info">Role:<span style={{fontWeight: "300"}}>{users.role}</span></p>
        ...
        <p className="user-info">Date:<span style={{fontWeight: "300"}}>{users.date.toDateString().slice(0,10) + ', ' + users.date.toDateString().slice(11)}</span></p>
      </Stack>
    </Card>
  );
};
